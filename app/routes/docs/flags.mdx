...
A flag `s` assim como a `m`, altera o comportamento de um meta caractere, que no caso é o `.`. O ponto significa qualquer coisa, exeto quebras de linha, com a flag `s`, o ponto passa a significar também as quebras de linha.

<Code language="js">

```js
const regex = /.+/g
const regexDotAll = /.+/gs
...
console.log(text.match(regexDotAll).length) // 1
```
